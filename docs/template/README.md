...
curl -X PUT http://localhost:3000/api/categories/68f1e930d03816679f514824 \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name": "Updated Name"}'
...
curl -X DELETE http://localhost:3000/api/categories/68f1e930d03816679f514824 \
  -H "Authorization: Bearer $TOKEN"
...
# Get second page
curl "http://localhost:3000/api/categories?page=2&limit=5" \
  -H "Authorization: Bearer $TOKEN"
```
...
curl "http://localhost:3000/api/categories?filter[name]=Electronics" \
  -H "Authorization: Bearer $TOKEN"

# Filter products by category
curl "http://localhost:3000/api/products?filter[categoryId]=68f1d03f81d7fb554e63e4d3" \
  -H "Authorization: Bearer $TOKEN"

# Filter active items only
curl "http://localhost:3000/api/categories?filter[isActive]=true" \
  -H "Authorization: Bearer $TOKEN"
```
...
curl "http://localhost:3000/api/categories?sort=-createdAt" \
  -H "Authorization: Bearer $TOKEN"

# Sort by name (ascending)
curl "http://localhost:3000/api/categories?sort=name" \
  -H "Authorization: Bearer $TOKEN"

# Multiple sort fields
curl "http://localhost:3000/api/categories?sort=-createdAt,name" \
  -H "Authorization: Bearer $TOKEN"
```
...
curl "http://localhost:3000/api/categories?page=2&limit=10&filter[isActive]=true&sort=name" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -i -H "x-correlation-id: my-custom-id" http://localhost:3000/api/categories
...
```bash
curl http://localhost:3000/api/categories/999999999999999999999999 \
  -H "Authorization: Bearer $TOKEN"
...
```bash
curl "http://localhost:3000/api/categories?page=1&limit=10&sort=-createdAt" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl "http://localhost:3000/api/products?filter[categoryId]=68f1d03f81d7fb554e63e4d3&page=1&limit=20" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X POST http://localhost:3000/api/reports/product-summary
```
