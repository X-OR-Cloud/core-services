...
export class NodeService extends BaseService<Node> {
  ...
  async getSetupGuide(id: string, os: string, context: RequestContext): Promise<SetupGuideResponseDto> {
    ...
    if (os === 'docker') {
      const fileContent = this.generateDockerScript(setupToken, baseApiUrl, baseWsUrl, monaBaseUrl);
      const installCommand = `chmod +x start-xnode.sh && ./start-xnode.sh`;
      const instructions = [
        `1. Save the file content below as "start-xnode.sh" on your Docker host.`,
      ...
      return { os, installCommand, instructions, setupTokenExpiresAt: expiresAt, fileName: 'start-xnode.sh', fileContent };
    }

    if (os === 'kubernetes') {
      const fileContent = this.generateK8sManifest(setupToken, baseApiUrl, baseWsUrl, monaBaseUrl);
      const installCommand = `kubectl apply -f xnode-k8s.yaml`;
      const instructions = [
        `1. Save the file content below as "xnode-k8s.yaml".`,
        `2. Run: ${installCommand}`,
        `3. The init container will bootstrap the node on first start and persist credentials to the PVC.`,
        `4. For subsequent restarts, the init container skips bootstrap automatically.`,
        `5. Setup token is valid for 24 hours and single-use. If the PVC is deleted, generate a new setup guide.`,
      ];
      this.logger.log('Setup guide generated (kubernetes)', { nodeId: id, requestedBy: context.userId, expiresAt });
      return { os, installCommand, instructions, setupTokenExpiresAt: expiresAt, fileName: 'xnode-k8s.yaml', fileContent };
    }
...
`;
  ...
  private generateK8sManifest(setupToken: string, apiBaseUrl: string, wsBaseUrl: string, monaBaseUrl: string): string {
    return `# =============================================================
# xnode-k8s.yaml — XNode Kubernetes Deployment
# Generated by AIWM — do not edit manually
# =============================================================
# Usage:
#   kubectl apply -f xnode-k8s.yaml
#
# Restart: kubectl rollout restart deployment/xnode
# Xóa:     kubectl delete -f xnode-k8s.yaml
# =============================================================

---
# ConfigMap — cấu hình môi trường
apiVersion: v1
kind: ConfigMap
metadata:
  name: xnode-config
...
---
# PersistentVolumeClaim — lưu trữ agents/logs/data/config
apiVersion: v1
...
spec:
  ...
  resources:
    requests:
      storage: 10Gi
  # storageClassName: standard   # bỏ comment nếu cluster cần chỉ định SC
...
spec:
  ...
  template:
    ...
    spec:
      ...
      # Init container: bootstrap lần đầu, bỏ qua nếu credentials đã tồn tại trên PVC
      initContainers:
        - name: xnode-init
          image: ${DOCKER_IMAGE}
          command: ["/bin/bash", "-c"]
          args:
            - |
              CRED_FILE=/etc/xnode/credentials
              if [ -f "\$CRED_FILE" ]; then
                echo "[init] Credentials found, skipping bootstrap"
              else
                echo "[init] Bootstrapping node..."
                if ! command -v jq &>/dev/null; then
                  apt-get update -qq && apt-get install -y -qq jq
                fi
                RESP=\$(curl -sf -X POST "\${AIWM_API_BASE_URL}/nodes/auth/bootstrap" \\
                  -H "Content-Type: application/json" \\
                  -d "{\\"setupToken\\":\\"\${SETUP_TOKEN}\\"}" || true)
                if [ -z "\$RESP" ]; then
                  echo "[init] Bootstrap failed. Token may be expired or already used."
                  exit 1
                fi
                NODE_API_KEY=\$(echo "\$RESP" | jq -r '.nodeId // empty')
                NODE_SECRET=\$(echo "\$RESP" | jq -r '.secret // empty')
                if [ -z "\$NODE_API_KEY" ] || [ -z "\$NODE_SECRET" ]; then
                  echo "[init] Bootstrap error: \$(echo "\$RESP" | jq -r '.message // "unknown"')"
                  exit 1
                fi
                echo "NODE_API_KEY=\$NODE_API_KEY" > "\$CRED_FILE"
                echo "NODE_SECRET=\$NODE_SECRET" >> "\$CRED_FILE"
                chmod 600 "\$CRED_FILE"
                echo "[init] Bootstrap successful"
              fi
          envFrom:
            - configMapRef:
                name: xnode-config
      ...
      containers:
        - name: xnode
          ...
          args:
            - |
              set -e
              set -a; source /etc/xnode/credentials; set +a
              echo "[start] Node.js: \$(node -v)"
              echo "[start] Downloading xnode from \$RELEASES_URL..."
              wget -q "\$RELEASES_URL/xnode-latest.tar.gz" -O /tmp/xnode.tar.gz
              mkdir -p /opt/xnode
              tar -xzf /tmp/xnode.tar.gz -C /opt/xnode
              echo "[start] Downloading any-agent-pi..."
              wget -q "\$RELEASES_URL/any-agent-pi-latest.tar.gz" -O /tmp/any-agent.tar.gz
              mkdir -p /opt/any-agent-pi
              tar -xzf /tmp/any-agent.tar.gz -C /opt/any-agent-pi --strip-components=1
              echo "[start] Starting XNode..."
          ...
          volumeMounts:
            - name: xnode-data
              mountPath: /usr/agents
              subPath: agents
            - name: xnode-data
              mountPath: /var/log/xnode
              subPath: logs
            - name: xnode-data
              mountPath: /var/lib/xnode
              subPath: data
            - name: xnode-data
              mountPath: /etc/xnode
              subPath: config
